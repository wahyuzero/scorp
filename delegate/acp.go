...
func runSubagentACP(params delegateTaskParams) delegateResult {
	...
	// Special handling for opencode - use CLI mode instead of ACP (ACP auth not implemented)
	if params.ACPCommand == "opencode" {
		return runOpenCodeCLI(params, subagentID, start)
	}
...
// runOpenCodeCLI executes a task via `opencode run` CLI.
func runOpenCodeCLI(params delegateTaskParams, subagentID string, start time.Time) delegateResult {
	// Build the full prompt with context
	fullPrompt := params.Task
	if params.Context != "" {
		fullPrompt = fmt.Sprintf("Context:\n%s\n\nTask:\n%s", params.Context, params.Task)
	...
	// Use model from config: zai-coding-plan/glm-5.1 (default) or override
	...
	if len(params.ACPArgs) > 0 {
		// Allow override of args (e.g., --continue, --session, --fork)
		args = append(args, params.ACPArgs...)
	}
	...
	if err != nil {
		return delegateResult{
			SubagentID: subagentID,
			Task:       params.Task,
			Role:       params.Role + " (opencode CLI)",
			ModelUsed:  model,
			Status:     "failed",
			Error:      fmt.Sprintf("opencode run failed: %v\nOutput: %s", err, string(output)),
			Duration:   duration,
		}
	}
