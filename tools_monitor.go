...
func loadMonitorTargets() {
	...
	path := scorpPath("monitor_targets.json")
	if data, err := os.ReadFile(path); err == nil {
		json.Unmarshal(data, &monitorTargets)
	}
...
func saveMonitorTargets() {
	...
	defer monitorTargetsMu.Unlock()
	path := scorpPath("monitor_targets.json")
	data, _ := json.MarshalIndent(monitorTargets, "", "  ")
...
func executeMonitor(args map[string]interface{}, chatID int64) (string, bool) {
	...
	case "add":
		...
		interval := getIntArg(args, "interval_min", 30)
		screenshot := getBoolArg(args, "screenshot", false)
		ingestRAG := getBoolArg(args, "ingest_rag", true)
...
func ragIngestText(text, source, url string) {
	...
	chunks := SmartChunk(text, 500)
	for _, chunk := range chunks {
		vecIndex.addVecChunk(source, chunk)
	}
	vecIndex.persist()
