...
func parseDelegateParams(args map[string]interface{}) delegateTaskParams {
	p := delegateTaskParams{
		...
		Tools:      getStringSliceArg(args, "tools"),
		MaxIters:   getIntArg(args, "max_iters", DefaultSubagentIters),
		ReturnRaw:  getBoolArg(args, "return_raw", false),
		Role:       getStringArg(args, "role", "auto"),
		Model:      getStringArg(args, "model", ""),
		ACPCommand: getStringArg(args, "acp_command", ""),
		ACPArgs:    getStringSliceArg(args, "acp_args"),
	}
...
func executeDelegateBatch(args map[string]interface{}) (string, bool) {
	...
	params.MaxBatch = getIntArg(args, "max_batch", MaxParallelSubagents)
	...
	for i, task := range params.Tasks {
		...
		go func(idx int, t delegateTaskParams) {
			...
			log.Printf("[delegate_batch] Subagent %d/%d started: %s",
				idx+1, len(params.Tasks), truncateStr(t.Task, 60))
			if t.ACPCommand != "" {
	...
	for i, r := range results {
		...
		sb.WriteString(fmt.Sprintf("%s <b>[%d] %s</b>", statusIcon, i+1, truncateStr(r.Task, 60)))
		if r.ModelUsed != "" {
			sb.WriteString(fmt.Sprintf(" <i>(%s)</i>", r.ModelUsed))
		}
		sb.WriteString("\n")
		sb.WriteString(fmt.Sprintf("   ⏱ %s | 🔧 %v\n", r.Duration.Round(time.Second), r.ToolsUsed))
		sb.WriteString(fmt.Sprintf("   %s\n\n", truncateStr(r.Result, 500)))
	}
