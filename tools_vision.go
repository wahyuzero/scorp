...
func executeAnalyzeImage(args map[string]interface{}) (string, bool) {
	...
	question := getStringArg(args, "question", "Describe this image in detail.")
	...
	return truncOutput(result, maxToolOutput), false
}
...
func callVisionModel(question, dataURL string) (string, error) {
	...
	resp, err := getAIClient(baseURL).Do(req)
	if err != nil {
	...
	if resp.StatusCode != 200 {
		return "", fmt.Errorf("vision API error (HTTP %d): %s", resp.StatusCode, truncateStr(string(body), 300))
	}
