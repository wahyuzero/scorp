...
// LoadMCPConfig reads ~/.scorp-agent/mcp.json
func LoadMCPConfig() (*MCPConfig, error) {
	path := os.ExpandEnv("$HOME") + "/.scorp-agent/mcp.json"
...
func registerMCPToolsAsNative() {
	...
	for serverName, srv := range mcpServers {
		for _, tool := range srv.tools {
			...
			// Build ArgDef from inputSchema for system prompt descriptions
...
// to a map of ArgDef entries for system prompt descriptions.
func buildArgDefsFromInputSchema(schema map[string]interface{}) map[string]ArgDef {
...
func (s *MCPServer) sendRequest(method string, params interface{}) (*jsonRPCResponse, error) {
	...
	for time.Now().Before(deadline) {
		...
		if err := json.Unmarshal([]byte(line), &resp); err != nil {
			log.Printf("[mcp] Skipping non-JSON line from %s: %s", s.Name, truncateStr(line, 100))
			continue
		}
...
func executeMCPServerTool(name string, args map[string]interface{}) (string, bool) {
	switch name {
	case "shell":
		return executeShell(args, 0)
	case "system_info":
		res, _ := executeSystemInfo(args)
		return res, true
	case "search_code":
		res, _ := executeSearchCode(args)
		return res, true
	case "log":
		res, _ := executeLog(args)
		return res, true
	default:
