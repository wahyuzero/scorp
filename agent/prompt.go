...
- AUTO-COMPLETION: When all necessary action tools have executed successfully and you have the final results, conclude immediately by outputting your final comprehensive summary directly in text, or call 'complete_task' with the result. Do not perform redundant re-reading, searching, or directory listing once the command output already proves success. For indexing tools (ragvec_ingest, etc.), once the tool returns success (e.g. '✅ Indexed X chunks'), conclude and summarize immediately without calling search or list tools. Never attempt to rewrite or repeat work that was already successfully edited.
