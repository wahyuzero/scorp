...
func SendMessageSmart(text string, keyboard map[string]interface{}) {
	if isAnyModeActive() {
		...
		log.Printf("[telegram] Message queued (mode active): %s", helpers.TruncateStr(text, 60))
		return
	}
	tools.SendMessage(text, keyboard)
}

// isAnyModeActive checks if any chat session has agent mode active
...
// historyDir is resolved dynamically via config.HistoryDirPath()

func historyFilePath(chatID string) string {
	return fmt.Sprintf("%s/%s.json", config.HistoryDirPath(), chatID)
}
...
func appendSessionHistory(chatID string, msgs ...AgentMessage) {
	...
	for _, msg := range msgs {
		if content, ok := msg.Content.(string); ok && content != "" {
			go session.IndexMessage(chatID, msg.Role, content)
		}
	}
...
func scorpChat(chatID, userMessage string) (string, error) {
	touchSession(chatID)
	log.Printf("[scorp] Chat request from %s: %s", chatID, helpers.TruncateStr(userMessage, 50))

	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancel()

	messages := []models.ChatMessage{
		{Role: "system", Content: "You are Scorp Agent, a helpful AI assistant. Respond in the same language as the user."},
		{Role: "user", Content: userMessage},
	}

	reply, _, err := models.CallModelWithFallback(ctx, "chat", messages)
	if err != nil {
		return "", err
	}
...
func scorpChatMultiTurn(messages []AgentMessage) (string, error) {
	...
	reply, _, err := models.CallModelWithFallback(ctx, "chat", chatMsgs)
	if err != nil {
		return "", err
	}
...
func sendScorpReply(chatID int64, msgID int64, reply string) {
	...
	if len(display) <= 3800 {
		tools.EditMessageByID(chatID, msgID, header+display, nil)
	} else {
		chunks := helpers.SplitMessage(display, 3800)
		tools.EditMessageByID(chatID, msgID, header+chunks[0], nil)
		for k := 1; k < len(chunks); k++ {
			time.Sleep(300 * time.Millisecond)
			tools.SendMessage(chunks[k], nil)
		}
	}
}
