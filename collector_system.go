...
func checkGDriveMount() GDriveInfo {
	info := GDriveInfo{Path: cfg.RcloneGDriveMount}
	fi, err := os.Stat(cfg.RcloneGDriveMount)
	if err != nil || !fi.IsDir() {
		return info
	}

	// Check if it's a mount point
	out, err := exec.Command("mountpoint", "-q", cfg.RcloneGDriveMount).CombinedOutput()
	_ = out
	info.Mounted = err == nil

	if info.Mounted {
		entries, err := os.ReadDir(cfg.RcloneGDriveMount)
		_ = entries
...
func checkS3Gateway() S3Info {
	info := S3Info{URL: cfg.RcloneS3GatewayURL}
	client := httpShort
	resp, err := client.Get(cfg.RcloneS3GatewayURL)
	if err != nil {
...
func getRcloneServiceStatus() RcloneInfo {
	...
	cachePath := os.ExpandEnv("$HOME/.cache/rclone")
