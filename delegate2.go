...
func runSubagent(params delegateTaskParams) delegateResult {
	...
	for iteration := 0; iteration < params.MaxIters; iteration++ {
		...
		if params.Model != "" {
			m := getModelByName(params.Model)
			if m != nil {
				text, err = callModel(ctx, m, chatMsgs)
				if err != nil {
					// Fallback to routed model
					text, toolCalls, modelLabel, err = callModelWithToolsAndFallback(ctx, taskType, chatMsgs)
				} else {
					modelLabel = m.Model
					toolCalls, _ = parseAllToolCalls(text, nil)
				}
			} else {
				text, toolCalls, modelLabel, err = callModelWithToolsAndFallback(ctx, taskType, chatMsgs)
			}
		...
		if len(toolCalls) == 0 {
			...
			toolCalls, text = parseAllToolCalls(text, nil)
		}
		...
		for _, tc := range toolCalls {
			...
			result, _ := executeTool(tc, 0) // chatID=0 for subagent (no Telegram)
