# skills.go

> 18 nodes

## Key Concepts

- **skills.go** (10 connections) — `skills/skills.go`
- **ExecuteSkillManage()** (7 connections) — `skills/manage.go`
- **skills/manage.go** (6 connections) — `skills/manage.go`
- **executeSkillManageCreate()** (5 connections) — `skills/manage.go`
- **executeSkillManageUpdate()** (5 connections) — `skills/manage.go`
- **GetByName()** (5 connections) — `skills/skills.go`
- **executeSkillManageDelete()** (4 connections) — `skills/manage.go`
- **Skill** (4 connections) — `skills/skills.go`
- **SanitizeName()** (4 connections) — `skills/skills.go`
- **Save()** (4 connections) — `skills/skills.go`
- **executeSkillManageList()** (3 connections) — `skills/manage.go`
- **ExecuteSkillManageView()** (3 connections) — `skills/manage.go`
- **GetAll()** (3 connections) — `skills/skills.go`
- **Load()** (3 connections) — `skills/skills.go`
- **Delete()** (2 connections) — `skills/skills.go`
- **GetPromptForMessage()** (1 connections) — `skills/skills.go`
- **HandleCommand()** (1 connections) — `skills/skills.go`
- **List()** (1 connections) — `skills/skills.go`

## Relationships

- [init](init.md) (1 shared connections)
- [startCLI](startCLI.md) (1 shared connections)
- [StartDaemon](StartDaemon.md) (1 shared connections)

## Source Files

- `skills/manage.go`
- `skills/skills.go`

## Audit Trail

- EXTRACTED: 27 (73%)
- INFERRED: 10 (27%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [index](index.md) to navigate.*