...
// FormatHourlyReport generates a comprehensive hourly report
func FormatHourlyReport(sys collectors.SystemData, docker collectors.DockerData, coolify collectors.CoolifyData,
	sec collectors.SecuritySummary, stor collectors.StorageData, net collectors.NetworkData) string {

	now := time.Now().UTC().Format("02 Jan 2006, 15:04 UTC")
	sections := []string{
		fmt.Sprintf("📊 <b>System Report</b>\n🕐 %s\n%s", now, strings.Repeat("─", 28)),
		SectionSystem(sys),
...
// FormatStatusResponse generates a quick status response
func FormatStatusResponse(sys collectors.SystemData, docker collectors.DockerData, stor collectors.StorageData) string {
...
func SectionSystem(d collectors.SystemData) string {
...
func SectionDocker(d collectors.DockerData) string {
...
func SectionCoolify(d collectors.CoolifyData) string {
	if !d.Available {
...
func SectionSecurity(d collectors.SecuritySummary) string {
	lines := []string{
	...
	if len(d.RecentFailed) > 0 {
		...
		for _, e := range d.RecentFailed {
			...
			geo := ""
			if e.Geo.Country != "" {
				geo = fmt.Sprintf(" %s %s", collectors.Flag(e.Geo.CC), e.Geo.Country)
				if e.Geo.City != "" {
					geo += ", " + e.Geo.City
				}
			}
...
func SectionStorage(d collectors.StorageData) string {
	gdIcon := "❌"
...
func SectionNetwork(d collectors.NetworkData) string {
	lines := []string{
		"<b>🌐 Network</b>",
...
// Bar generates a Unicode progress bar
