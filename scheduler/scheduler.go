...
type ScheduledTask struct {
	...
	ChatTarget      int64  `json:"chat_target,omitempty"`
	...
	PrevResult      string `json:"prev_result,omitempty"`
}
...
// LoadTasks loads scheduled tasks from config store
func LoadTasks() {
	scheduledTasksMu.Lock()
	defer scheduledTasksMu.Unlock()

	if err := config.ConfigMgr().Load("scheduler.json", &scheduledTasks); err != nil {
...
func saveTasks() {
	if err := config.ConfigMgr().Save("scheduler.json", scheduledTasks); err != nil {
...
// AddTask adds a scheduled task with basic params
func AddTask(name, taskType, schedule, prompt string) (*ScheduledTask, error) {
	return AddTaskEx(name, taskType, schedule, prompt, nil)
}

// AddTaskEx adds a scheduled task with optional per-job config
func AddTaskEx(name, taskType, schedule, prompt string, args map[string]interface{}) (*ScheduledTask, error) {
	...
	if args != nil {
		task.MaxRetries = helpers.GetIntArg(args, "max_retries", 0)
		task.Timeout = helpers.GetIntArg(args, "timeout", 0)
		task.ChainContext = helpers.GetBoolArg(args, "chain_context", false)
		task.NotifyOnError = helpers.GetBoolArg(args, "notify_on_error", false)
		task.NotifyOnSuccess = helpers.GetBoolArg(args, "notify_on_success", false)
	}
...
func ToggleTask(id string, enabled bool) bool {
	scheduledTasksMu.Lock()
	defer scheduledTasksMu.Unlock()
...
func GetTask(id string) *ScheduledTask {
	scheduledTasksMu.Lock()
	defer scheduledTasksMu.Unlock()
	for i, t := range scheduledTasks {
		if t.ID == id {
...
// ShellTask runs a shell command directly
func ShellTask(command string) (string, string) {
	...
	if err != nil {
		msg := fmt.Sprintf("⏰ <b>Scheduled Task (Error)</b>\n\n"+
			...
			"<b>Error:</b> %s\n\n<pre>%s</pre>",
			helpers.EscapeHTML(helpers.TruncateStr(command, 100)),
...
func runAgentTask(prompt string) (string, string) {
	...
	msgID := int64(0)
	if SendMessageGetID != nil {
		msgID = SendMessageGetID(fmt.Sprintf("⏰ <i>Scheduled: %s</i>", helpers.EscapeHTML(helpers.TruncateStr(prompt, 80))), chatID)
	}
	if msgID == 0 {
...
// NextRunTime calculates the next run time from a schedule expression
...
// ExecuteSchedule handles schedule CRUD from agent tool calls
func ExecuteSchedule(args map[string]interface{}) (string, bool) {
	action := helpers.GetStringArg(args, "action", "")

	switch action {
	case "add":
		name := helpers.GetStringArg(args, "name", "unnamed")
		taskType := helpers.GetStringArg(args, "type", "agent")
		schedule := helpers.GetStringArg(args, "schedule", "")
		prompt := helpers.GetStringArg(args, "task", "")
		...
		task, err := AddTaskEx(name, taskType, schedule, prompt, args)
		if err != nil {
		...
		return fmt.Sprintf("✅ Scheduled task created:\n"+
			...
			helpers.TruncateStr(task.Prompt, 100)), true
	...
	case "list":
		...
		for _, t := range scheduledTasks {
			...
			sb.WriteString(fmt.Sprintf("%s [%s] %s\n  Schedule: %s | Next: %s\n  Last: %s | Runs: %d\n  Task: %s\n\n",
				...
				lastInfo, t.RunCount,
				helpers.TruncateStr(t.Prompt, 60)))
		}
...
// FormatTasksList returns a formatted HTML list of scheduled tasks
func FormatTasksList() string {
	scheduledTasksMu.Lock()
	defer scheduledTasksMu.Unlock()
