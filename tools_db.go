...
var dbConnectionsFile = "/root/.scorp-agent/db_connections.json"
...
func executeSQL(args map[string]interface{}, chatID int64) (string, bool) {
	...
	rowLimit := getIntArg(args, "limit", 100)
	if rowLimit > 1000 {
		rowLimit = 1000
	}

	// Determine connection
	connName := getStringArg(args, "connection", "default")
	dbType := getStringArg(args, "db_type", "")
	dsn := getStringArg(args, "dsn", "")
	...
	if dbType == "" || dsn == "" {
		...
		if conn, ok := conns[connName]; ok {
			...
			return "Error: no connection specified. Provide db_type + dsn, or configure ~/.scorp-agent/db_connections.json", false
		}
	}
