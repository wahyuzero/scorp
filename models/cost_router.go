...
func LoadCostConfig() {
	...
	if err := config.ConfigMgr().Load("cost_config.json", &costCfg); err != nil {
		log.Printf("[cost] Load error: %v", err)
		costCfg = defaultCostConfig()
		return
	}
	if costCfg == nil {
...
func saveCostConfig() {
	...
	if err := config.ConfigMgr().Save("cost_config.json", costCfg); err != nil {
		log.Printf("[cost] Save error: %v", err)
	}
}
...
func LoadCostTracker() {
	...
	var ct CostTracker
	if err := config.ConfigMgr().Load("cost_daily.json", &ct); err != nil {
...
func saveCostTracker() {
	costTracker.mu.Lock()
	defer costTracker.mu.Unlock()
	if err := config.ConfigMgr().Save("cost_daily.json", costTracker); err != nil {
		log.Printf("[cost] Save error: %v", err)
	}
}
...
func handleCostCommand(args map[string]interface{}) (string, bool) {
	sub := helpers.GetStringArg(args, "cost_action", "")
	...
	case "budget":
		amount := helpers.GetFloatArg(args, "amount", 5.0)
		costCfgMu.Lock()
	...
	case "offpeak":
		model := helpers.GetStringArg(args, "offpeak_model", "")
		start := int(helpers.GetFloatArg(args, "offpeak_start", 22))
		end := int(helpers.GetFloatArg(args, "offpeak_end", 7))
	...
	case "rate":
		model := helpers.GetStringArg(args, "rate_model", "")
		inCost := helpers.GetFloatArg(args, "input_cost", 0)
		outCost := helpers.GetFloatArg(args, "output_cost", 0)
