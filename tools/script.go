...
func ExecuteScript(args map[string]interface{}, chatID int64) (string, bool) {
	...
	inline := helpers.GetStringArg(args, "inline", "")
...
func ExecuteScriptList(args map[string]interface{}, chatID int64) (string, bool) {
	scriptsDir := config.ScorpPath("scripts")
	files, err := os.ReadDir(scriptsDir)
	if err != nil {
		return "No scripts directory. Create one at ~/.scorp/scripts/", true
	}
