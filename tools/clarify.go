...
func executeClarify(args map[string]interface{}) (string, bool) {
	...
	msgID := sendClarifyMessage(chatID, question, choices)
	if msgID == 0 {
	...
	pendingClarifies[chatIDStr] = &PendingClarify{
		ChatID:   chatIDStr,
		Question: question,
		Choices:  choices,
		Response: respChan,
	...
	case resp := <-respChan:
		// Clean up
		pendingClarifiesMu.Lock()
		delete(pendingClarifies, chatIDStr)
		pendingClarifiesMu.Unlock()
...
func sendClarifyMessage(chatID int64, question string, choices []string) int64 {
	if len(choices) > 0 {
		...
		msgID := SendMessageGetID("🤔 <b>Klarifikasi</b>\n\n"+question, chatID)
		if msgID != 0 {
			EditMessageByID(chatID, msgID, "🤔 <b>Clarification</b>\n\n"+question, keyboard)
		}
		return msgID
	}

	// Open-ended: just send message, wait for text reply
	return SendMessageGetID("🤔 <b>Clarification</b>\n\n"+question+"\n\n<i>Reply with your answer.</i>", chatID)
}
