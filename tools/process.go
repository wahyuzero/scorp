...
func ExecuteProcess(args map[string]interface{}, chatID int64) (string, bool) {
	...
	switch action {
	case "list":
		filter := helpers.GetStringArg(args, "filter", "")
		if filter != "" {
			cmd = exec.CommandContext(ctx, "bash", "-c",
				fmt.Sprintf("ps aux | grep -i '%s' | grep -v grep | head -30", filter))
		} else {
			// Use compatible ps flags: --sort with rss (memory) or pcpu (cpu)
			cmd = exec.CommandContext(ctx, "bash", "-c",
				"ps aux --sort=-rss | head -20")
		}

	case "top":
		sortBy := helpers.GetStringArg(args, "sort_by", "mem")
		switch sortBy {
		case "cpu":
			cmd = exec.CommandContext(ctx, "bash", "-c",
	...
	case "kill":
		pid := helpers.GetStringArg(args, "pid", "")
		if pid == "" {
