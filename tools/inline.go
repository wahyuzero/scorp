...
func buildInlineResults(query string) []map[string]interface{} {
	...
	default:
		...
		if title, ok := safeCmds[query]; ok {
			...
			text := fmt.Sprintf("<b>%s</b>\n<code>%s</code>", title, helpers.EscapeHTML(output))
			if errOut != "" {
				text = fmt.Sprintf("❌ <b>Error</b>\n<code>%s</code>", helpers.EscapeHTML(errOut))
			}
			results = append(results, map[string]interface{}{
				"type":        "article",
				"id":          "cmd_" + query,
...
func quickStorage() string {
	...
	return "📁 <b>Storage</b>\n<code>" + helpers.EscapeHTML(strings.TrimSpace(string(out))) + "</code>"
}
...
func SetupInlineMode() {
	TgPost("/setMyDescription", map[string]interface{}{
		"description": "Scorp Agent — Smart AI Assistant. Type @botname in any chat to get started.",
	})
	TgPost("/setMyName", map[string]interface{}{
		"name": "Scorp Agent",
	})
	TgPost("/setMyShortDescription", map[string]interface{}{
		"short_description": "AI Assistant: type @botname to ask or run commands",
	})
